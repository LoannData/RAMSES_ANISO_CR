...
subroutine conduction_fine(ilevel,compute,itemp)
  ...
  !--------------------------------------------------------------------------
  integer::i,ivar,igrid,ncache,ngrid,ind,iskip,icpu,compute
...
subroutine condfine1(ind_grid,ncache,ilevel,compute,itemp)
  ...
  real(dp)::Cvi,Te,Ti,Tau_ei_vol,n_ion,n_electron,Tau_equi_reduced
 ...
 ! Integer constants
  ...
  if(semi_implicit)then
     call cond_split_semi(uloc,flux,dx,dx,dx,dt_imp,ncache,compute,facdx,itemp)
  else
     call cond_split(uloc,flux,dx,dx,dx,dt_imp,ncache,compute,facdx,itemp)
  endif
#ifndef WITHOUTMPI
  tc3=MPI_WTIME()
  dtc23=dtc23+tc3-tc2
