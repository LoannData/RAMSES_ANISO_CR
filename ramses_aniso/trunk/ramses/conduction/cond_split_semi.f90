...
! ----------------------------------------------------------------
subroutine cond_split_semi(uin,flux,dx,dy,dz,dt,ngrid,compute,fdx,itemp)
  use amr_parameters
  use const             
...
!  use radiation_parameters
  ...
  real(dp)::mu,scale_TK,kappa_coef
  real(dp)::eps,Tp_loc,rho
  ...
  if(compute==0)then
     ...
     call cmpXheatflx_trans(Temp,dens,bf,kspitzer,Xflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
     do k=klo,khi
        do j=jlo,jhi
           do i=if1,if2
              do l = 1, ngrid
     ...
     call cmpYheatflx_trans(Temp,dens,bf,kspitzer,Yflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
     do k=klo,khi
        do j=jf1,jf2
           do i=ilo,ihi
              do l = 1, ngrid
     ...
     call cmpZheatflx_trans(Temp,dens,bf,kspitzer,Zflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
     do k=kf1,kf2
        do j=jlo,jhi
           do i=ilo,ihi
              do l = 1, ngrid
     ...
     ! Compute the heat flux in X direction  
     call cmpXheatflx_norm(Temp,dens,bf,kspitzer,Xflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
     do k=klo,khi
        do j=jlo,jhi
           do i=if1,if2
              do l = 1, ngrid
...
#if NDIM>1
     ! Compute the heat flux in Y direction
     call cmpYheatflx_norm(Temp,dens,bf,kspitzer,Yflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
     do k=klo,khi
        do j=jf1,jf2
           do i=ilo,ihi
              do l = 1, ngrid
...
#if NDIM>2
     ! Compute the heat flux in Z direction
     call cmpZheatflx_norm(Temp,dens,bf,kspitzer,Zflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
     do k=kf1,kf2
        do j=jlo,jhi
           do i=ilo,ihi
              do l = 1, ngrid
...
!#########################################################################
!#########################################################################
subroutine cmpXheatflx_trans(Temp,dens,bf,kspitzer,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
  use amr_parameters
  use const             
  use hydro_parameters
  implicit none 
...
!#########################################################################
!#########################################################################
subroutine cmpYheatflx_trans(Temp,dens,bf,kspitzer,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
  use amr_parameters
  use const             
  use hydro_parameters
  implicit none 
...
!#########################################################################
!#########################################################################
subroutine cmpZheatflx_trans(Temp,dens,bf,kspitzer,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
  use amr_parameters
  use const             
  use hydro_parameters
  implicit none 
...
!###########################################################
!###########################################################
subroutine cmpXheatflx_norm(Temp,dens,bf,kspitzer,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
  use amr_parameters
  use const             
  use hydro_parameters
  implicit none 
...
!#########################################################################
!#########################################################################
subroutine cmpYheatflx_norm(Temp,dens,bf,kspitzer,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
  use amr_parameters
  use const             
  use hydro_parameters
  implicit none 
  ...
  real(dp)::Bnorm,fy,oneoverdx,oneovertwodx,oneoverfourdx,famr
  real(dp)::Bnorm2_1,Bnorm2_2,Bnorm2_3,Bnorm2_4
  real(dp)::dTdx1,dTdx2,dTdx3,dTdx4
  real(dp)::dTdy1,dTdy2,dTdy3,dTdy4
  real(dp)::dTdz1,dTdz2,dTdz3,dTdz4
...
!#########################################################################
!#########################################################################
subroutine cmpZheatflx_norm(Temp,dens,bf,kspitzer,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,Temp2)
  use amr_parameters
  use const             
  use hydro_parameters
  implicit none 
  ...
  real(dp)::Bnorm,fz,oneoverdx,oneovertwodx,oneoverfourdx,famr
  real(dp)::Bnorm2_1,Bnorm2_2,Bnorm2_3,Bnorm2_4
  real(dp)::dTdx1,dTdx2,dTdx3,dTdx4
  real(dp)::dTdy1,dTdy2,dTdy3,dTdy4
  real(dp)::dTdz1,dTdz2,dTdz3,dTdz4
