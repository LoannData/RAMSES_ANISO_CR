subroutine conduction_cg (ilevel,Nsub,itemp)
  use amr_commons
  use hydro_commons
  ...
  !===========================================================================
  integer,intent(IN)::ilevel,Nsub,itemp
  ...
  real(dp)::alpha_cg,beta_cg,Cv,told,tnew,wdt,rho
  ...
  integer :: this,iter_nr,nx_loc,nsub_imp
  ...
  real(dp)::Z_i,electron_charge,electron_mass,Tau_eH,Tau_ei_vol,Te_new,Te_old,Cvi,Cve,Tau_equi_reduced
  real(dp)::scale_tau,Ti_new,Ti_old,T_new
  real(dp)::n_electron,n_ion,cfl,cfl_loc,dt_cond,temp_loc
  real(dp)::tt1,tt2,tt1b,tt2b,deltatt_cond,tt1iter,tt1start,tt2end,deltatt_start,deltatt_end
...
!!$  endif
  ...
  if(semi_implicit)then 
     ...
     call cmp_matrixA_cond (ilevel,Nsub, 0,itemp)
     do i=1,nb_ind
  ...
  !===================================================
  call cmp_matrixA_cond (ilevel,Nsub, 1,itemp)
  ...
  !ben
  call make_boundary_conduction(ilevel,itemp)
  call cmp_matrixA_cond (ilevel,Nsub, 3,itemp)
  ...
  tt1iter=MPI_WTIME()
#endif
  do while(ratio>epsilon .and.iter<itermax)
...
!!$        tt1b=MPI_WTIME()
!!$#endif
     call cmp_matrixA_cond (ilevel,Nsub, 2,itemp)
!!$#ifndef WITHOUTMPI
!!$        call MPI_BARRIER(MPI_COMM_WORLD,info)
!!$        tt2b=MPI_WTIME()
...
!###########################################################
!###########################################################
subroutine cmp_matrixA_cond (ilevel,Nsub,compute,itemp)
  !------------------------------------------------------------------
  ...
  integer,intent(IN)::compute,ilevel,Nsub,itemp

  integer ,dimension(1:nvector,0:2*ndim)::  igridn
...
subroutine compute_Tau_equi_vol(Tau_ei_vol)
  use amr_commons,ONLY:dp
  use hydro_commons,only:coulomb_log,f_spitzer,mu_electron
...
subroutine make_boundary_conduction(ilevel,itemp)
  ...
  real(dp)::dd,t2,usquare,emag,erad_loc,eps,ekin,ecr,Cv,rho
  real(dp)::scale_kappa
  real(dp)::scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2
  real(dp)::a_R,radiation_Source,kpara_ana,Cv1,Cv1_electron,Cv1_ion
  integer ::ind_eint,ind_Tenew,ind_Teold,ind_Cve,igroup
