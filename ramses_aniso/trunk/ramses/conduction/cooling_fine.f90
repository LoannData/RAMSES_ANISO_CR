...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  !-------------------------------------------------------------------
  !-------------------------------------------------------------------
  integer::i,ind,iskip,idim,nleaf,nx_loc,ix,iy,iz,ivar,irad,ind_cr
  ...
  real(kind=8),dimension(1:nvector),save::nH,T2,delta_T2,ekk,err,errcr
  ...
  real(kind=8)::Tau_ei_reduced,Tau_ie_reduced,dFdx11,dFdx12,dFdx21,dFdx22,F1,F2,dtratio1,dtratio2
  real(kind=8)::dFdx11inv,dFdx12inv,dFdx21inv,dFdx22inv
  real(kind=8)::oneoverdet,dTi,dTe,Ti2,Te2,error,tcr_cool,tcr_heat
  integer::niterimp

  ind_cr=9
  if(twotemp)ind_cr=10
  ...
  do ind=1,twotondim
     ...
     do irad=0,ncr-1
        do i=1,nleaf
           errcr(i)=errcr(i)+uold(ind_leaf(i),ind_cr+irad)
        end do
     end do
#endif
     if(twotemp)then
        do i=1,nleaf
           T2(i)=(gamma_rad(1)-1.0)*uold(ind_leaf(i),9)*mu_electron/mu_gas
        end do
     else
        do i=1,nleaf
           T2(i)=(gamma-1.0)*(T2(i)-ekk(i)-emag(i)-err(i)-errcr(i))
        end do
     endif
     ...
     if(cooling.or.neq_chem)then
        if(twotemp)then
           do i=1,nleaf
              delta_T2(i) = delta_T2(i)*nH(i)/scale_T2/(gamma_rad(1)-1.0)
           end do
        else
           do i=1,nleaf
     ...
     if(twotemp)then
        ! WARNING : This is wrong. Modify to account for all energies in twotemp case
        do i=1,nleaf
           T2min(i) = T2min(i)*nH(i)/scale_T2/(gamma_rad(1)-1.0)
        end do
     else
        do i=1,nleaf
     ...
     if(twotemp)then
        do i=1,nleaf
           T2(i) = uold(ind_leaf(i),9)*mu_electron/mu_gas
        end do
     else
        do i=1,nleaf
           T2(i) = uold(ind_leaf(i),5)
     ...
     if(cooling.or.neq_chem)then
        ...
        if(twotemp)then
           do i=1,nleaf
              T2(i) = T2(i)*mu_gas/mu_electron
           end do
        endif
     endif
     if(isothermal)then
        do i=1,nleaf
           uold(ind_leaf(i),5) = T2min(i) + ekk(i) + err(i) + errcr(i) + emag(i)
        end do
     else if(cooling .or. neq_chem)then
        if(twotemp)then
           do i=1,nleaf
              uold(ind_leaf(i),5) = uold(ind_leaf(i),5) - uold(ind_leaf(i),9)
              uold(ind_leaf(i),9) = max(T2(i),T2min(i))
              uold(ind_leaf(i),5) = uold(ind_leaf(i),5) + uold(ind_leaf(i),9)
           end do           
        else
           do i=1,nleaf
              uold(ind_leaf(i),5) = T2(i) + T2min(i) + ekk(i) + err(i) + errcr(i) + emag(i)
           end do
        endif
     endif
     ...
     if(cooling_cr)then
        do i=1,nleaf
           ...
           uold(ind_leaf(i),ndim+2) = uold(ind_leaf(i),ndim+2)-errcr(i) &
                & + errcr(i)*( exp(-dtcool/tcr_cool) + tcr_cool/tcr_heat*(1d0-exp(dtcool/tcr_cool)) )
           uold(ind_leaf(i),ind_cr) = errcr(i)*exp(-dtcool/tcr_cool) ! YD: only work for one CR bin
        end do
     endif
