...
subroutine crdiff_split(uin,flux,dx,dy,dz,dt,ngrid,compute,fdx,igroup) !YD:test
     use amr_parameters
     use const             
   ...
   !  use radiation_parameters
     ...
     ! Input states
     integer,dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2)::indpass !YD:test
     ...
     real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2),save::Ecr,Ecr0,Dpara,kperp,Vstr
     ...
     i1=MAX(iu1,0); i2=MIN(iu2,3)
     j1=MAX(ju1,0); j2=MIN(ju2,3)
     k1=MAX(ku1,0); k2=MIN(ku2,3)
...
!!$     do i = iu1, iu2
     ...
     do i = i1, i2
        do l = 1, ngrid
           ...
           if(compute==2)Ecr(l,i,j,k)=uin(l,i,j,k,1) !! 2 Anciennement 
    ...
    ! Compute the heat flux in X direction
     call cmpXcrflx(Ecr,Ecr0,bf,Xflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,Vstr)
   !!$  call cmpXcrflx(Ecr,Ecr0,bf,Xflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,indpass) ! YD:test
     do k=klo,khi
     do j=jlo,jhi
     do i=if1,if2
        do l = 1, ngrid
     ...
     call cmpYcrflx(Ecr,Ecr0,bf,Yflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,Vstr)
   !!$  call cmpYcrflx(Ecr,Ecr0,bf,Yflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,indpass) ! YD:test
     do k=klo,khi
     do j=jf1,jf2
     do i=ilo,ihi
        do l = 1, ngrid
     ...
     call cmpZcrflx(Ecr,Ecr0,bf,Zflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,Vstr)
   !!$  call cmpZcrflx(Ecr,Ecr0,bf,Zflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,indpass) ! YD:test
     do k=kf1,kf2
     do j=jlo,jhi
     do i=ilo,ihi
        do l = 1, ngrid
   ...
   !#########################################################################
   !#########################################################################
   subroutine cmpXcrflx(Temp,Temp0,bf,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,Vstr)
   !!$subroutine cmpXcrflx(Temp,Temp0,bf,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,indpass) !YD:test
     use amr_parameters
     use const             
     use hydro_parameters
     implicit none 
     ...
     real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2)::Temp,Temp0,Dpara,kperp,Vstr
   ...
   !#########################################################################
   !#########################################################################
   subroutine cmpYcrflx(Temp,Temp0,bf,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,Vstr)
   !!$subroutine cmpYcrflx(Temp,Temp0,bf,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,indpass) !YD:test
     use amr_parameters
     use const             
     use hydro_parameters
     implicit none 
   ...
   !#########################################################################
   !#########################################################################
   subroutine cmpZcrflx(Temp,Temp0,bf,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,Vstr)
   !!$subroutine cmpZcrflx(Temp,Temp0,bf,myflux,dx,dy,dz,dt,ngrid,compute,ffdx,kpar,Dpara,kperp,indpass) !YD:test
     use amr_parameters
     use const             
     use hydro_parameters
     implicit none 
   ...
   subroutine MonCen_lim(a,b,a2,b2,rr1,moncenresult)
     use amr_parameters,only :dp
     implicit none
     real(dp)::a,b,a2,b2,aa,bb,r1,moncenresult,rr1,rr2,aa2,bb2,r2
