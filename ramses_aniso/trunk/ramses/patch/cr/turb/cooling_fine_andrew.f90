...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  real(kind=8)::oneoverdet,dTi,dTe,Ti2,Te2,error
  integer::niterimp
  ...
  do ind=1,twotondim
     ...
     if(low_density_isothermal)then
        ! Set to T2_star - NOT COMPATIBLE WITH isothermal or barotrop
        do i=1,nleaf
            T2min(i) = T2_star
        end do
     endif
     ...
     if (low_density_isothermal)then
        do i=1,nleaf
           if (nH(i) <= low_density_threshold .OR. T2(i) < T2min(i)) then
              uold(ind_leaf(i),5) = T2min(i)
           end if
        end do
     else if(isothermal)then
        do i=1,nleaf
           uold(ind_leaf(i),5) = T2min(i)
        end do
     else
