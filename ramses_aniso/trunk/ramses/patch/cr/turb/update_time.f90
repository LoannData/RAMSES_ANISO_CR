...
subroutine update_time(ilevel)
  ...
  if(nstep_coarse .ne. nstep_coarse_old)then
     ...
     if(const==0.0D0)then
        ...
        econs=(ekin_tot+epot_tot-epot_tot_int-const-eturb_ever) / &
             &(-(epot_tot-epot_tot_int-const-eturb_ever)+ekin_tot)
     end if
     ...
     if(mod(nstep_coarse,ncontrol)==0.or.output_done)then
        if(myid==1)then
           ...
           if(cooling.or.pressure_fix)then
              ...
              write(*,780)nstep_coarse,econs,epot_tot,ekin_tot,eint_tot!,erad_tot
              write(*,781)emag_tot,eturb_ever,ekin_tot-eturb_ever,mcons
           else
              write(*,777)nstep_coarse,mcons,econs,epot_tot,ekin_tot
           end if
           write(*,*) 'Maximum density: ', maxrho
#ifdef SOLVERmhd
...
999 format(' Level ',I2,' has ',I10,' grids (',3(I8,','),')')
 780 format(' Main step=',i6,' econs=',1pe13.5,' epot=',1pe13.5,&
         & ' ehydro=',1pe13.5,' eint=',1pe13.5)!, ' erad=', 1pe13.5)
781 format(' emag=',1pe13.5,' eturb=',1pe13.5,' ehydro - eturb=',1pe13.5,&
         & ' mcons=',1pe13.5)
