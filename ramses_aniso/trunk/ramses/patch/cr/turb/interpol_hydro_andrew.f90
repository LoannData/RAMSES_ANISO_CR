...
subroutine upl(ind_cell,ncell)
  ...
  !---------------------------------------------------------------------
  integer ::ivar,i,idim,ind_son,iskip_son,ind,neul=5,n
  integer ,dimension(1:nvector),save::igrid_son,ind_cell_son
  real(dp),dimension(1:nvector),save::getx,ekin,emag,erad_loc
  ...
  if(interpol_var==1 .or. interpol_var==2)then
     ...
     do ind_son=1,twotondim
        ...
        erad_loc(1:ncell)=0.0d0
        do n=1,ngrp
           do i=1,ncell
              erad_loc(i)=erad_loc(i) + uold(ind_cell_son(i),8+n)
           end do
        end do
  
        ! Update average
        do i=1,ncell
           getx(i)=getx(i)+uold(ind_cell_son(i),neul)-ekin(i)-emag(i)-erad_loc(i)
        end do
     end do
     ...
     do n=1,ngrp
        do i=1,ncell
           erad_loc(i)=erad_loc(i) + uold(ind_cell(i),8+n)
        end do
     end do

     ! Scatter result to cells
     do i=1,ncell
        uold(ind_cell(i),neul)=getx(i)/dble(twotondim)+ekin(i)+emag(i)+erad_loc(i)
     end do
