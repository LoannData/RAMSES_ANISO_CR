SUBROUTINE read_turb_fields
   ...
   character(len=17)  :: eturb_ever_z     ! eturb_ever in hex
   ...
   read(eturb_ever_z, '(Z16)') eturb_ever
   
   if (reset_time) then
       ! The time would have been old_t but has been reset to zero
       turb_last_time = turb_last_time - old_t
       turb_next_time = turb_next_time - old_t
   end if
