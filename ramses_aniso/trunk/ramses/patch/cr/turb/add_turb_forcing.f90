...
subroutine add_turb_forcing(ilevel, dteff)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        call turb_forcing1(ind_cell,ngrid,dteff,e_add)
        eturb_tot = eturb_tot + e_add*vol
     end do
     ! End loop over cells
...
!################################################################
!################################################################
subroutine turb_forcing1(ind_cell,ncell,dteff,e_add)
  use amr_commons
  use hydro_commons
  use turb_commons
  implicit none
  ...
  do idim=1,ndim
     do i=1,ncell
        pp(i)=uold(ind_cell(i),idim+1)+ &
             & uold(ind_cell(i),1)*fturb(ind_cell(i),idim)*dteff
     end do
     do i=1,ncell
  ...
  do i=1,ncell
     uold(ind_cell(i),neul)=pp(i)
     e_after = e_after + pp(i)
  end do
  
  e_add = e_after - e_before
