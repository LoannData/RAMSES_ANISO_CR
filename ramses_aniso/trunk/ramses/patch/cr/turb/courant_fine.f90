subroutine courant_fine(ilevel)
  ...
  real(kind=8)::mass_loc,ekin_loc,eint_loc,emag_loc,dt_loc,maxrho_loc
  real(kind=8)::mass_all,ekin_all,eint_all,emag_all,dt_all,maxrho_all
  real(kind=8),dimension(5)::comm_buffin,comm_buffout
 ...
 maxrho_all=0.0d0; maxrho_loc=0.0d0
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim        
        ...
        do ivar=1,3
           do i=1,nleaf
              emag_loc=emag_loc+0.125d0*(uu(i,5+ivar)+uu(i,nvar+ivar))**2*vol+0.5d0*uu(i,1+ivar)**2/uu(i,1)*vol
           end do
        end do
        ...
        maxrho_loc = max(maxrho_loc, maxval(uu(1:nleaf, 1)))
  ...
  comm_buffin(5)=eturb_tot
  call MPI_ALLREDUCE(comm_buffin,comm_buffout,5,MPI_DOUBLE_PRECISION,MPI_SUM,&
       &MPI_COMM_WORLD,info)
  call MPI_ALLREDUCE(dt_loc     ,dt_all      ,1,MPI_DOUBLE_PRECISION,MPI_MIN,&
       &MPI_COMM_WORLD,info)
  call MPI_REDUCE(maxrho_loc    ,maxrho_all  ,1,MPI_DOUBLE_PRECISION,MPI_MAX,&
       &0, MPI_COMM_WORLD,info)
  mass_all=comm_buffout(1)
  ekin_all=comm_buffout(2)
  eint_all=comm_buffout(3)
  emag_all=comm_buffout(4)
  eturb_tot=comm_buffout(5)
#endif
#ifdef WITHOUTMPI
  mass_all=mass_loc
  ekin_all=ekin_loc
  ...
  eturb_ever=eturb_ever+eturb_tot
  eturb_tot=0.0D0
  dtnew(ilevel)=MIN(dtnew(ilevel),dt_all)
  maxrho = max(maxrho, maxrho_all)
