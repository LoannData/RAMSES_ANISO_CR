...
!###########################################################
!###########################################################
subroutine cmpdt(uu,gg,xx,dx,dt,ncell)
  use amr_parameters
  ...
  real(dp):: dt_sh1,dt_sh2,dt_sh,Omeg_shear,dt_old
  integer:: n_min
  ...
  do k = 1,ncell
     ...
     dt = min(dt,dtcell)
     if(dtcell .eq. dt) n_min=k
  end do



  !add courant condition for shear 
  if (shear) then
  Omeg_shear = Vshear / boxlen
     do k = 1, ncell
        !courant condition along y
        dt_sh1 = max( sqrt(a2(k)) , abs(uu(k,3)) ) / abs(-uu(k,2) + Omeg_shear*xx(k,2)) / Omeg_shear
!        dt_sh2 = 2. * max( a2(k) , min(abs(uu(k,2)),abs(uu(k,3))) ) / max(abs(uu(k,2)),abs(uu(k,3)))  / Omeg_shear
        !courant condition along x
        dt_sh2 = max( sqrt(a2(k)) , abs(uu(k,2)) ) / abs(uu(k,3))  / Omeg_shear

        dt_sh = min(dt_sh1,dt_sh2) / 2. 

!        if(dt .ge. dt_sh) then
!          write(*,*) 'dt, dt_sh1, dt_sh2 ',dt, dt_sh1,dt_sh2
!          write(*,*) 'omeg_shear ', Omeg_shear
!          write(*,*) 'xx, uu ', xx(k,2), uu(k,2),uu(k,3)
!          write(*,*) 'Cs, rho ',a2(k),uu(k,1)
!        endif
...
!  if(dt .lt. dt_old) then
!    write(*,*) 'dt_old, dt,xx ',dt_old, dt,xx(n_min,2)
!    write(*,*) 'rho, vx,vy,vz,cs',uu(n_min,1),uu(n_min,2),uu(n_min,3),a2(n_min)
!    write(*,*) 'bx,by,bz',uu(n_min,6),uu(n_min,7),uu(n_min,8)
!  endif
