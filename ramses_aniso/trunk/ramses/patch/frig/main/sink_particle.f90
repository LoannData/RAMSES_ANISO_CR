...
subroutine grow_sink(ilevel,on_creation)
  ...
  do isink=1,nsink
     ...
     ! Remove mass ejected by jet
     if (jet_feedback_sink.and.ok_jet(isink)) msink(isink)=msink(isink)-delta_mass(isink)*frac_acc_ej
     ...
     if ((agn.and.ok_blast_agn(isink)).or.(jet_feedback_sink.and.ok_jet(isink)))then
...
subroutine accrete_sink(ind_grid,ind_part,ind_grid_part,ng,np,ilevel,on_creation)
  ...
  real(dp)::fbk_mass_jet, fbk_mom_jet, tan_theta_j
  ...
  ! geometry safety net for jet
  cone_jet = max(tiny(0.0),cone_jet)
  cone_jet = min(cone_jet, 180.d0)
  tan_theta_j=tan(3.1415926/180.*cone_jet/2) ! tangent of half of the opening angle
  ...
  do ind=1,twotondim
     do j=1,np
        if(ok(j,ind))then
           ...
           ! HACK - THIS IS ANNOYING IF THRESHOLD ACCRETION SET
           ...
           if (on_creation)then
              ...
              if(jet_feedback_sink)then
                  if(ok_jet(isink))then
                      ! Jet feedback momentum (according to Wang et al. 2010)
                      fbk_mass_jet=delta_mass(isink)*frac_acc_ej*weight/volume*d/density*(180./cone_jet)
                      fbk_mom_jet=fbk_mass_jet*v_jet*(msink(isink)*scale_d*scale_l**3/2.d33)**expo_jet/scale_v
                  end if
              end if
           ...
           if (on_creation)then
              ...
              if(jet_feedback_sink)then
                  if(ok_jet(isink))then
                     ! checking if particle is in cone
                     if (sum(lsink(isink,1:3)**2).gt.0)then
                         cone_dir(1:3)=lsink(isink,1:3)/sqrt(sum(lsink(isink,1:3)**2))
                     else
                         cone_dir = (/1,0,0/) !assign random direction if angular momentum is 0
                     end if
                     cone_dist=sum(r_rel(1:3)*cone_dir(1:3))
                     orth_dist=sqrt(sum((r_rel(1:3)-cone_dist*cone_dir(1:3))**2))
                     if (orth_dist.le.abs(cone_dist)*tan_theta_j)then
                        d=max(unew(indp(j,ind),1),smallr)
                        vv(1:3)=unew(indp(j,ind),2:4)/d
                        e=0.d0 !energy that is independent of the density
#ifdef SOLVERmhd
                        bx1=unew(indp(j,ind),6)
                        by1=unew(indp(j,ind),7)
                        bz1=unew(indp(j,ind),8)
                        bx2=unew(indp(j,ind),nvar+1)
                        by2=unew(indp(j,ind),nvar+2)
                        bz2=unew(indp(j,ind),nvar+3)                        
                        e=e+0.125d0*((bx1+bx2)**2+(by1+by2)**2+(bz1+bz2)**2)
#endif
#if NENER>0
                        do irad=0,nener-1
                           e=e+unew(indp(j,ind),inener+irad)
                        end do
#endif
                        unew(indp(j,ind),1)=d+fbk_mass_jet/vol_loc  !add ejected mass to cell
                        unew(indp(j,ind),2:4)=unew(indp(j,ind),2:4)+fbk_mom_jet*r_rel(1:3)/(ir_cloud*dx_min)/vol_loc &
                          & +vsink(isink,1:3)*fbk_mass_jet/vol_loc !add momentum to cell
                        unew(indp(j,ind),5)=(unew(indp(j,ind),5)-e-0.5d0*d*sum(vv(1:3)**2))/d*unew(indp(j,ind),1)
                        unew(indp(j,ind),5)=unew(indp(j,ind),5)+e+0.5d0*sum(unew(indp(j,ind),2:4)**2)/unew(indp(j,ind),1)
                     end if
                  end if
              end if
           end if
...
!              end if
        ...
        if(jet_feedback_sink)then
               !if(myid==1) write(*,*) "jet_feedback_sink check if ok_jet",isink, ok_jet(isink)
               ! check whether we should have jet feedback
               ok_jet(isink)=.false.
               if((msink(isink)*scale_d*scale_l**3/2d33.ge.mass_jet_sink).and.(delta_mass(isink)/msink(isink).ge.1.d-3).and.(volume_gas(isink).gt.0.))then
                   ok_jet(isink)=.true.
               end if
               !if(myid==1) write(*,*) "ok_jet", isink, ok_jet(isink)
               if(myid==1.and.verbose_jet.and.ok_jet(isink))then
                   write(*,'("***jet***",I4,1X,4(1PE12.5,1X))') "sink no. ", isink &
                        & , "Mass ", msink(isink)*scale_d*scale_l**3/2d33 &
                        & , "dM ", delta_mass(isink)*scale_d*scale_l**3/2d33
               endif
        end if
     end if
  end do
...
subroutine print_sink_properties(dMEDoverdt,rho_inf,r2,v_bondi)
  ...
  if (.not. smbh)then    
     if(myid==1.and.nsink>0.and. mod(nstep_coarse,ncontrol)==0)then
        ...
        write(*,'(" ================================================================================================================================================================== ")')
        write(*,'("   Id     M[Msol]  delta_M[Msol]  dmfsink[Msol]         x             y             z            vx            vy            vz    acc_rate[Msol/y] acc_lum[Lsol]     age      ")')
        write(*,'(" ================================================================================================================================================================== ")')
        do i=nsink,1,-1
           isink=idsink_sort(i)
           l_abs=(lsink(isink,1)**2+lsink(isink,2)**2+lsink(isink,3)**2)**0.5+1.d10*tiny(0.d0)
           write(*,'(I5,12(2X,E12.5))')&
                idsink(isink),msink(isink)*scale_m/1.9891d33,delta_mass(isink)*scale_m/1.9891d33,dmfsink(isink)*scale_m/1.9891d33,&
