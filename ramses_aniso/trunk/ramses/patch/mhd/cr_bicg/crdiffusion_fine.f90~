        ...
        do ivar=1,nvar+3
           do i=1,nexist
              ...
              if(compute==2 .and. ivar==1)then 
                 uloc(ind_exist(i),i3,j3,k3,ivar)=unew(ind_cell(i),3) ! y of PBiCGSTAB
                 ! neighbor cell at ilevel+1, put value to zero in vector
                 ! because it is considered as a Dirichlet BC, i.e. in the RHS
                 if(son(ind_cell(i))>0) uloc(ind_exist(i),i3,j3,k3,ivar)=0.0d0
              endif
              if(compute==4 .and. ivar==2)then 
                 uloc(ind_exist(i),i3,j3,k3,ivar)=unew(ind_cell(i),8)  ! z of PBiCGSTAB
                 ! neighbor cell at ilevel+1, put value to zero in vector
                 ! because it is considered as a Dirichlet BC, i.e. in the RHS
                 if(son(ind_cell(i))>0) uloc(ind_exist(i),i3,j3,k3,ivar)=0.0d0
              endif
           ...
           do i=1,nbuffer
              ...
              ! neighbor cell at ilevel+1, put value to zero in vector
              ! because it is considered as a Dirichlet BC, i.e. in the RHS
              if(compute==2 .and. ivar==1)uloc(ind_nexist(i),i3,j3,k3,ivar)=0.0
              if(compute==4 .and. ivar==2)uloc(ind_nexist(i),i3,j3,k3,ivar)=0.0
           end do
        end do
  ...
  if(streaming_diffusion)then
     ...
     do l=1,ncache
        do i1=0,3
           ...
           uloc(l,i1,j1,k1,3)=va*length*gamma_rad(1)
        enddo
     enddo
#endif     
#if NDIM==2
           ...
           do i=1,ncache
              if(son(ind_cell(i))==0)then
                 if(compute==1)then
                    ...
                    unew(ind_cell(i),5     ) = -residu_loc(i,i3   ,j3   ,k3  ) ! rzero of PBiCGSTAB
                 else if(compute==2)then
