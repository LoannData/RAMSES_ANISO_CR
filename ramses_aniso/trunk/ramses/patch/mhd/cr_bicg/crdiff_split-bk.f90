...
!!$        1-------
        ...
        if(compute .ne. 3)then   
           if(do_limiter_aniso)then
              ...
              call L2_lim(a,b,alpha_limiter,dTdx1)
              b=Temp(l,i,j+1,k)-Temp(l,i-1,j+1,k)
              call L2_lim(a,b,alpha_limiter,dTdx2)
...
!!$              TL=Temp(l,i,j-1,k+1)-Temp(l,i-1,j-1,k+1)
!!$              TC=Temp(l,i,j  ,k+1)-Temp(l,i-1,j  ,k+1)
!!$              TR=Temp(l,i,j+1,k+1)-Temp(l,i-1,j+1,k+1)
!!$              ML=Temp(l,i,j-1,k  )-Temp(l,i-1,j-1,k  )
!!$              MC=Temp(l,i,j  ,k  )-Temp(l,i-1,j  ,k  )
!!$              MR=Temp(l,i,j+1,k  )-Temp(l,i-1,j+1,k  )
!!$              BL=Temp(l,i,j-1,k-1)-Temp(l,i-1,j-1,k-1)
!!$              BC=Temp(l,i,j  ,k-1)-Temp(l,i-1,j  ,k-1)
!!$              BR=Temp(l,i,j+1,k-1)-Temp(l,i-1,j+1,k-1)
...
!!$        1------2
        ...
        if(compute .ne. 3)then
           if(do_limiter_aniso)then
              ...
              call L2_lim(a,b,alpha_limiter,dTdy1)
              b=Temp(l,i+1,j,k)-Temp(l,i+1,j-1,k)
              call L2_lim(a,b,alpha_limiter,dTdy2)
...
!###########################################################
!###########################################################
subroutine L2_lim(a,b,alpha,l2result)
  use amr_parameters,only :dp
  implicit none
  real(dp)::a,b,alpha,l2result
  ...
  l2result= MAX(mymin,MIN(mean,mymax))
...
!###########################################################
!###########################################################
subroutine MinMod_lim(a,b,minmodresult)
  use amr_parameters,only :dp
  use const         ,only :one
  implicit none
  real(dp)::a,b,ab,minmodresult,dsign
  ...
  if(ab.le.0d0)then 
     ...
     dsign=sign(one,a)
     minmodresult= dsign*MIN(abs(a),abs(b))
  endif
...
!###########################################################
!###########################################################
subroutine MonCen_lim(a,b,moncenresult)
  use amr_parameters,only :dp
  implicit none
  real(dp)::a,b,aa,bb,r1,moncenresult
  
  call MinMod_lim(a,b,r1)
  aa=2d0*r1
  bb=0.5d0*(a+b)
  call MinMod_lim(aa,bb,moncenresult)
