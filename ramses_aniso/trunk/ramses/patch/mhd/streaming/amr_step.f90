recursive subroutine amr_step(ilevel,icount)
  use amr_commons
  use amr_parameters,only:cr_streaming
  use pm_commons
  use hydro_commons
  use poisson_commons
#ifdef RT
  ...
  integer::mpi_err
#endif
  integer::ilevel,icount,ilev,icr
  !-------------------------------------------------------------------!
...
#if NCR>0
  ! Cosmic rays streaming
  if(cr_streaming)then
     call set_dflux_crstreaming(ilevel)
  end if
#endif
  ...
  ! Cosmic rays streaming step
#if NCR>0
  if(cr_streaming)then
                             call timer('cr - streaming','start')
     call crstreaming_fine(ilevel)
     call set_uold_cr(ilevel)
