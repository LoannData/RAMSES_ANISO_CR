subroutine hydro_flag(ilevel)
  ...
  real(dp),dimension(1:nvector,1:nvar+4),save::uug,uum,uud
  ...
  if(    err_grad_d==-1.0.and.&
       ...
       & err_grad_B2==-1.0.and.&
       & err_grad_u==-1.0.and.&
       & err_grad_vort==-1.0.and.&
#if NENER>0
       & sum(err_grad_prad(1:NENER)).eq.-nener .and.&
#endif
#if NVAR>8+NENER
       &sum(err_grad_var(1:NVAR-8-NENER)).eq.-(nvar-8-nener) .and.&
#endif
     ...
     do ind=1,twotondim
        ...
        do idim=1,ndim
           ...
           if(err_grad_vort.ge.0.0d0)then
              do i=1,ngrid
                 uug(i,nvar+4)=enew(indn(i,2*idim-1))
                 uum(i,nvar+4)=enew(ind_cell(i     ))
                 uud(i,nvar+4)=enew(indn(i,2*idim  ))
              end do
           endif
