...
subroutine hydro_refine(ug,um,ud,ok,nn,ilevel)
  ...
  real(dp)::ug(1:nvector,1:nvar+4)
  real(dp)::um(1:nvector,1:nvar+4)
  real(dp)::ud(1:nvector,1:nvar+4)
  ...
  if(err_grad_vort >= 0.)then
     do k=1,nn
        vg=ug(k,nvar+4); vm=um(k,nvar+4); vd=ud(k,nvar+4)
        if(vm>floor_vort)then
           error=2.0d0*MAX( &
                & ABS((vd-vm)/(vd+vm+1d-10)) , &
                & ABS((vm-vg)/(vm+vg+1d-10)) )
!!$           error=2.0d0*MAX( &
!!$                & ABS((vd-vm)/(vd+vm+floor_vort)) , &
!!$                & ABS((vm-vg)/(vm+vg+floor_vort)) )
           ok(k) = ok(k) .or. error > err_grad_vort
        endif
     end do
  end if
  if(ivar_refine>0)then
     do k=1,nn
        ok(k)=ok(k).and.um(k,ivar_refine)/um(k,1)>var_cut_refine
     enddo
  endif
