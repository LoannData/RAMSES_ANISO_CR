...
subroutine flag
  use amr_commons
  use hydro_commons,only: err_grad_vort
  implicit none
  integer::ilevel

  if(verbose)write(*,*)'Entering flag'
  if(err_grad_vort.ge.0.0d0)then
     do ilevel=nlevelmax,1,-1
        call cmp_vorticity(ilevel)
     end do
  endif
...
!#####################################################################
!#####################################################################
subroutine cmp_vorticity(ilevel)
  use amr_commons
  use hydro_commons
  implicit none
  integer::ilevel
  ...
  real(dp)::dx,dx_loc,dx2_loc,scale
  real(dp),dimension(1:3)::skip_loc
  real(dp),dimension(1:twotondim,1:3)::xc
  real(dp),dimension(1:nvector,1:ndim),save::xx
  integer::ivx,ivy,ivz,ixl,ixr,iyl,iyr,izl,izr
  integer::indxl,indxr,indyl,indyr,indzl,indzr
  real(dp)::vortx,vorty,vortz
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,scale_vkms

  ! Conversion factor from user units to cgs units                              
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  scale_vkms=scale_v/1d5
...
!!$        end do
        ...
        ! Compute vorticity
        ivx=2;ivy=3;ivz=4
        ixl=1;ixr=2;iyl=3;iyr=4;izl=5;izr=6
        do i=1,ngrid
           indxl=indn(i,ixl);indxr=indn(i,ixr)
           indyl=indn(i,iyl);indyr=indn(i,iyr)
#if NDIM > 2
           indzl=indn(i,izl);indzr=indn(i,izr)
           vortx=(uold(indyr,ivz)/uold(indyr,1)-uold(indyl,ivz)/uold(indyl,1)) - &
                &(uold(indzr,ivy)/uold(indzr,1)-uold(indzl,ivy)/uold(indzl,1))
           vorty=(uold(indzr,ivx)/uold(indzr,1)-uold(indzl,ivx)/uold(indzl,1)) - &
                &(uold(indxr,ivz)/uold(indxr,1)-uold(indxl,ivz)/uold(indxl,1))
           vortz=(uold(indxr,ivy)/uold(indxr,1)-uold(indxl,ivy)/uold(indxl,1)) - &
                &(uold(indyr,ivx)/uold(indyr,1)-uold(indyl,ivx)/uold(indyl,1))
           enew(ind_cell(i))=sqrt(vortx*vortx+vorty*vorty+vortz*vortz)*scale_vkms
#else
           vortz=(uold(indxr,ivy)/uold(indxr,1)-uold(indxl,ivy)/uold(indxl,1)) - &
                &(uold(indyr,ivx)/uold(indyr,1)-uold(indyl,ivx)/uold(indyl,1))
           enew(ind_cell(i))=sqrt(vortz*vortz)*scale_vkms
